...
export function StatBar({ label, value, color }: StatBarProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 w-48 ${isLow ? "animate-pulse" : ""}`}
    >
      <span
        className={`text-sm font-mono w-20 ${isLow ? "text-red-500 font-bold" : ""}`}
      >
        {label}
      </span>
      <Progress
      ...
      />
      <span
        className={`text-sm font-mono w-8 text-right ${isLow ? "text-red-500 font-bold" : ""}`}
      >
        {value}
      </span>
    </div>
